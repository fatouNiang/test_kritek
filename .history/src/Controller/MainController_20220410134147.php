...
class MainController extends AbstractController
{
    ...
    public function index(Request $request, ManagerRegistry $doctrine): Response
    {
        ...
        if ($request->isMethod('POST') && $form->handleRequest($request) && $form->isValid()) {
            ...
            foreach($lines as $singleLine ){
                $amount_without_TVA = $singleLine->getQuantity() * $singleLine->getAmount();
                $amount_TVA = ($amount_without_TVA * $TVA) / 100;
                $total = $amount_TVA + $amount_without_TVA;
                $singleLine->setAmountTVA($total);
                $singleLine->setAmountTVA($amount_TVA);
    ...
    //     //          $amount_without_TVA = $singleLine->getQuantity() * $singleLine->getAmount();
    //     //          $amount_TVA = ($amount_without_TVA * $TVA) / 100;
    //     //          $total = $amount_vat + $amount_without_TVA;
